    ...
    public function getWeightedParkings(Request $request)
    {
        ...
        if (!empty($request->Userlat) && !empty($request->Userlong) && !empty($request->Destinationlat) && !empty($request->Destinationlog) && (isset($request->selection)
            ...
            foreach ($coordinates as $key => $value) {
                ...
                $coordinates[$key]['distancedestinationPossition'] = empty($matrix['distances'][1][$counter]) ? -1 : $matrix['distances'][1][$counter];
                $coordinates[$key]['durationcurrentPossition'] = empty($matrix['durations'][0][$counter]) ? -1 : $matrix['durations'][0][$counter];
                $coordinates[$key]['durationdestinationPossition'] = empty($matrix['durations'][1][$counter]) ? -1 : $matrix['durations'][1][$counter];
            ...
            foreach ($coordinates as $key => $value) {
                ...
                $coordinates[$key]['FuzzyValueTotal'] = $fuzzycalculation->calculateWithParameters($arrayofParameters);
            }
            ...
            $userRequest->totaltime = $end->diff($start)->format("%f");
            $userRequest->fuzzytime = $fuzzyend->diff($fuzzyStart)->format("%f");
